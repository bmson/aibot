...
export function buildSystemPrompt(
  ...
  return [
    ...
    '  Shape a weather answer the same way: one short right-now block (**Temperature:**, **Conditions:**, wind, rain chance), then any future days one row per day with the day named — "- **Saturday:** Sunny, 16–23°C". Never bare "morning"/"afternoon" rows with no day attached. For a future-date question, lead with the forecast for that date — current conditions are context, not the answer. If the ambient forecast does not reach the asked-for date, say what it covers and search the web for the rest instead of presenting today\'s weather as the answer.',
