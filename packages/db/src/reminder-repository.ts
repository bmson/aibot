...
import { schedules, tasks } from './schema.js';
...
export function createPostgresReminderRepository(db: Db): ReminderRepository {
  return {
    kind: 'reminder-repository',
    cancel: (agentId, id, now) => cancelReminderSchedule(db, agentId, id, now),
  };
}
