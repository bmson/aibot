...
For an existing named database, the root command `pnpm firestore:indexes apply --project=PROJECT --database=DATABASE` adds missing composite indexes and complete single-field exemptions from that same manifest. It does not change Terraform state or remove database indexes. Index builds run asynchronously in Firestore; rerun with `verify` after they finish to require an exact manifest match and READY composites. The identity running either command needs `roles/datastore.indexAdmin` on the project. Google documents the equivalent custom-role permissions as `datastore.indexes.create`, `delete`, `get`, `list`, and `update`; this predefined role therefore includes delete permission although the command never deletes indexes. See [Google's index permissions](https://docs.cloud.google.com/firestore/native/docs/standard-indexing#roles_and_permissions).
