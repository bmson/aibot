...
- The scheduled `memory.consolidate` worker now selects bounded owner-scoped facts through `FirestoreMemoryConsolidationRepository`, applies model decisions and any derived occasions atomically behind the privacy-erasure fence, then compiles the owner card through Firestore. Its PostgreSQL execution path remains unchanged. This migration does not move memory extraction or other memory jobs.
...
For an existing named database, the root command `pnpm firestore:indexes apply --project=PROJECT --database=DATABASE` adds missing composite indexes and complete single-field exemptions from that same manifest. It does not change Terraform state or remove database indexes. It submits each change with `--async` and returns without waiting for backfills; rerun with `verify` after Google reports them READY to require exact manifest parity. The identity running either command needs `roles/datastore.indexAdmin` on the project. Google documents the equivalent custom-role permissions as `datastore.indexes.create`, `delete`, `get`, `list`, and `update`; this predefined role therefore includes delete permission although the command never deletes indexes. See [Google's index permissions](https://docs.cloud.google.com/firestore/native/docs/standard-indexing#roles_and_permissions).
...
After configuring Application Default Credentials and enabling the Firestore API in that project, add `--run`. The command creates a new named Standard/Native database, provisions the versioned task/outbox indexes concurrently, runs the synthetic task smoke flow, records the runtime due-query's Query Explain metrics, and deletes its database afterward. It waits for every index operation to settle before cleanup, even after a build failure, and reports workload failures before waiting for deletion. Progress distinguishes index readiness, workload success/failure, and completed deletion; cloud operations can take several minutes. It requires database/index administration in the selected project and uses billable resources. It does not read your current workspace, use `(default)`, or create service-account keys. On an interrupted process or failed cleanup, use the exact database name printed in the progress log to remove the remaining validation resource.
