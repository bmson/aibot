...
describe.skipIf(!process.env.FIRESTORE_EMULATOR_HOST)('Firestore Gmail sync', { timeout: 30_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    for (const [email, trust] of [
      ...
      const id = randomUUID();
      await store.doc('contacts', id).set({ id, name: email, trust, emails: [email], aliases: [] });
    }
  ...
  it('baselines, then triages known mail once per message and advances the cursor', async () => {
    ...
    expect(ingest.docs.map((doc) => [doc.get('channelMessageId'), doc.get('importance')])).toEqual([
      ['gmail:m1', 4],
    ]);
