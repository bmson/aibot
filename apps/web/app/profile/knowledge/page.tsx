  ...
  return (
    <article className={`${cardShellClass} p-4 sm:p-5`}>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        ) : finding.kind === 'projection_failed' ? (
        ...
        {finding.memoryId ? (
          <form action={forgetKnowledgeMemory.bind(null, finding.memoryId)}>
            <ConfirmButton
              size="sm"
              confirmLabel="Forget source knowledge?"
              pendingLabel="Forgetting…"
              title="Deletes and tombstones the source memory, then removes its derived graph connections."
            >
              Forget knowledge
            </ConfirmButton>
          </form>
        ...
        <section className="mt-7">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <div className={segmentedControlClass}>
              {MEMORY_STATES.map((item) => (
                <Link
                  key={item}
                  href={hrefFor({ view: 'library', state: item })}
          ...
          >
            ...
            >
              ...
              {['identity', 'work', 'home', 'relationships', 'preferences', 'health', 'other'].map(
                (domain) => (
          ...
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            {library.rows.map(({ memory, subjectId, subjectLabel }) => (
              <FactRow
                key={memory.id}
                fact={toFactView(memory, subjectId, subjectLabel, ownerId, now)}
        ...
        <section className="mt-7">
          ...
          {map.truncated ? (
            <p className="mt-3 text-xs text-muted">
              Showing a connected 200-item overview from {map.totalEdges.toLocaleString()} matching
              connections. Narrow the filters to focus another component.
            </p>
          ) : null}
      ...
      {view === 'cleanup' ? (
        <section className="mt-7 max-w-4xl">
          ...
          <div className="mt-5 grid gap-3">
            {findings.length > 0 ? (
              findings.map((finding) => <CleanupCard key={finding.id} finding={finding} />)
            ) : (
              <EmptyState>
