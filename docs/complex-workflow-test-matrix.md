...
- Cross-event correlation from a later third-party confirmation email back to the exact application
  record. Unknown inbound email is correctly unable to mutate private Docs/Sheets today. Safe
  automation needs a first-class application record plus strict sender/job/confirmation matching;
  broad permission for arbitrary email-driven writes is not acceptable.
