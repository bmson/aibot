...
| Forged deep Authentication-Results header | Only the receiver's top-most header authenticates; a sender-injected mx.google.com header lower in the list never grants trust | `packages/modules/src/google/email-sync.test.ts` |
...
| Confirmation sender is spoofed/wrong, token is absent, watch expired/cancelled, or one email matches multiple applications | Make no private mutation; ambiguous matches become needs-attention and identify no application as confirmed | `apps/agent/src/application-confirmations.e2e.test.ts`, `packages/modules/src/google/email-sync.test.ts` |
...
| Approval or owner ping fires at the daily budget cap | The out-of-band SMS uses the same critical carve-out as final replies and still delivers | `packages/modules/src/sms/sms-critical.test.ts` |
...
| An email-originated follow-up (adhoc/scheduled) task finishes | Its final reply returns to the original Gmail thread — thread and authenticated recipient recovered from the conversation binding and the original email task — not only the dashboard | `packages/modules/src/google/email-channel.test.ts` (D5) |
| A known contact's email is answered (or clarified) with no outbound tool call | The dashboard-only dead-end is closed: an assistant-trust, taint-carrying adhoc child proposes exactly that reply via gmail.send, which gates on owner approval (approve → the exact draft is sent to the thread; deny → nothing sent); the owner-only auto-send invariant holds and unknown senders never get a child | `apps/agent/src/known-sender-reply.e2e.test.ts` (D9) |
