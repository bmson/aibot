...
Repeat the identical command to resume from persisted stages. Terraform state is remote and the last local completed stage is updated atomically. If creation of the state bucket succeeds but upload of its ownership receipt fails, the next run stops for manual ownership verification; it must not automatically adopt that unverified bucket. Failed Terraform work directories are retained for recovery. `provisioned` means the index resources were included in a successful Terraform apply; it does not verify live query readiness or advance the runtime to `initialized` or `ready`. An archive from before index packaging fails the new trusted-file check and requires an explicit recovery decision, rather than silently treating its prior `provisioned` stage as index-ready.
