...
The merged recovery resolver then restored those 12 verified generations create-only to their intended workspace paths. The first run copied 12 objects totaling 43,639,071 bytes with no conflicts; an independent generation-pinned read reproduced all 12 recovery SHA-256 hashes and byte counts. A second run copied nothing and verified all 12 existing destinations, demonstrating idempotence without overwrite.

Five references still have no byte candidate in the searched local files, customer-owned buckets, and retained object versions: two import originals and three generated artifacts. Derived memories survive for the missing imports, and the audit found no confirmed missing primary user upload. The historical objects had no trusted original checksums in the snapshot, so their recovery hashes establish baselines rather than proof against the original uploads. The 12 restored paths must be included in the final backup/restore inventory. Search another owner backup or device for the remaining five, or document accepted loss, before claiming every asset migrated.
