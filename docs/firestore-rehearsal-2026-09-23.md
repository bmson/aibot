...
The follow-up private revision `assistant-web-firestore-rehearsal-00004-59p` used the profile fix at PR head `5175c4bffbd6c418e84e4370c97399d81baefe35`. Authenticated mobile workspace returned `200` in 7.45 s (88,651 response bytes), including two current chats and its bounded 80 owner facts; bootstrap remained `200` in 2.84 s. This resolves the observed profile overflow for the imported data. It does not cover web owner sign-in, live chat/model response, mutation paths, background deliveries, or disabled modules, and the people/library latency findings remain.
...
- Measure and reduce the slow people and memory-library reads under the imported history, and exercise authenticated owner mutations, chat/model response, and enabled background paths against the private runtime.
