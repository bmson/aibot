...
| `email.extract` | email-extraction | **Ported but not admitted.** `firestore-email-extraction.test.ts` runs it through `persistence.emailExtraction`: facts go through the Firestore memory writer (embedding space, content-hash and tombstone markers), and occasions stay quarantined. It is missing from `FIRESTORE_PORTABLE_CODE_JOBS`, so `firestoreCodeJobUnavailable` still skips it. |
...
| `documents.process` | document-processing (every 15 min) | **Ported but not admitted.** `firestore-document-processing.test.ts` runs the launch, the one-shot callback and the hand-off to extraction through `persistence.documentProcessor`. It is missing from `FIRESTORE_PORTABLE_CODE_JOBS`, so `firestoreCodeJobUnavailable` still skips it. |
...
`sqlOnlyCodeJobs()` currently returns `documents.process` and `email.extract`. Both ports landed (466c490b, 7b2d3f9b), but each commit added its job to `CODE_JOBS` a second time instead of adding it to `FIRESTORE_PORTABLE_CODE_JOBS`. Until those two names move into the portable set, the Firestore sweep advances their schedules without creating tasks, and a queued one completes benignly (`firestoreCodeJobUnavailable`). The emulator tests pass because they call the job without the executor's `jobUnavailable` guard. Once they are admitted, no code job is SQL-only. `firestoreCodeJobUnavailable` and `sqlOnlyCodeJobs()` stay as a guard that names any future job added without a port (`firestore-sweep.test.ts`, `executor-deps.test.ts`).
...
Also outstanding: `documents.process` and `email.extract` need adding to `FIRESTORE_PORTABLE_CODE_JOBS` (see Code jobs) so the Firestore runtime runs their ports.
